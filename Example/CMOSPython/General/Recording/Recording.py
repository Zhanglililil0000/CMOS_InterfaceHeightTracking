...
def work_thread(cam=0, pData=0, nDataSize=0):
    ...
    stInputFrameInfo = MV_CC_INPUT_FRAME_INFO_EX()
    memset(byref(stInputFrameInfo), 0 ,sizeof(MV_CC_INPUT_FRAME_INFO_EX))
    while True:
        ret = cam.MV_CC_GetImageBuffer(stOutFrame, 1000)
        if None != stOutFrame.pBufAddr and 0 == ret:
            print ("get one frame: Width[%d], Height[%d], nFrameNum[%d]"  % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
            stInputFrameInfo.enPixelType = stOutFrame.stFrameInfo.enPixelType
            stInputFrameInfo.nWidth = stOutFrame.stFrameInfo.nExtendWidth
            stInputFrameInfo.nHeight = stOutFrame.stFrameInfo.nExtendHeight
            ...
            ret = cam.MV_CC_InputOneFrameEx(stInputFrameInfo)
            if ret != 0:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if ret != 0:
            raise Exception ("get AcquisitionFrameRateEnable fail! ret[0x%x]" % ret)
        ...
        if ret != 0: 
            raise Exception ("get width fail! nRet [0x%x]" % ret)
        ...
        if ret != 0: 
            raise Exception ("get height fail! nRet [0x%x]"% ret)
        ...
        if ret != 0: 
            raise Exception ("get PixelFormat fail! nRet [0x%x]" % ret)
        ...
        if ret != 0: 
            raise Exception ("get ResultingFrameRate value fail! nRet [0x%x]" % ret)
        ...
        if ret != 0: 
            raise Exception ("Start Record fail! nRet [0x%x]\n", nRet)
        ...
        if ret != 0:
            raise Exception ("stop Record fail! ret[0x%x]" % ret)
